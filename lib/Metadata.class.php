	...
	public function getTweet($link_id) {
		try {
			$sql = "
				...
				LIMIT 1;
			";
			$params = array('link_id' => array('value' => $link_id));
			...
			if (empty($id_str) || $id_str == NULL) {
				...
				if (preg_match("/2../", $twitter->http_code)) {
					...
					try {
						...
						";
						$params = array(
							'link_id' => array('value' => $link_id),
							'id_str' => array('value' => $id_str),
							'screen_name' => array('value' => $screen_name),
							'text' => array('value' => $text),
							'profile_image_url' => array('value' => $profile_image_url)
						);
						$sth = $this->execute($sql, $params);
